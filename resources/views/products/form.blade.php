                ...
                <form method="POST" action="{{ route('products.store') }}">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" id="name" name="name" class="mt-1 block w-full" value="{{ old('name') }}" placeholder="Digite o nome do produto" required>
                        </div>

                        <div>
                            <label for="description" class="block font-medium text-gray-700">{{ __('Descrição') }}</label>
                            <input type="text" id="description" name="description" class="mt-1 block w-full" value="{{ old('description') }}" placeholder="Digite uma descrição do produto">
                        </div>

                        <div>
                            <label for="price" class="block font-medium text-gray-700">{{ __('Preço') }}</label>
                            <input type="number" id="price" name="price" class="mt-1 block w-full" value="{{ old('price') }}" placeholder="Digite o preço do produto" required>
                        </div>

                        <div>
                            <label for="stock" class="block font-medium text-gray-700">{{ __('Estoque') }}</label>
                            <input type="number" id="stock" name="stock" class="mt-1 block w-full" value="{{ old('stock') }}" placeholder="Digite a quantidade em estoque" required>
                        </div>
                    </div>
